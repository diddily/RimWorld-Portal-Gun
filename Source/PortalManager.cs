			...
			if (dirty)
			{
				...
				foreach (Building_PortalGunPortalEntry entry in Portals)
				{
					foreach (Building_PortalGunPortalEntry exit in Portals)
					{
						...
						Log.Message(entry.Position + " to " + exit.Position + " path is " + DistanceLookup[entry][exit] + "("+ DistanceLookup[entry][exit].GetCost(DefaultCardinalCost, DefaultDiagonalCost)+")");
					}
				}
			}
