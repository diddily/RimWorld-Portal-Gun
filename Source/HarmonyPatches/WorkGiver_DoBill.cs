...
namespace Portal_Gun.HarmonyPatches
{
	...
	{
		private static ThingDef VanometricPowerCell = ThingDef.Named("VanometricPowerCell");
		private static void Postfix(WorkGiver_DoBill __instance, Bill bill, Pawn pawn, ref List<ThingCount> chosen, ref bool __result)
		{
			...
			{
				Thing thing = pawn.Map.spawnedThings.FirstOrDefault((Thing t) => t.GetInnerIfMinified().def == VanometricPowerCell && bill.IsFixedOrAllowedIngredient(t) && pawn.CanReach(t, PathEndMode.Touch, Danger.Deadly) && !t.IsForbidden(pawn) && t is MinifiedThing);
				if (thing != null)
				{
